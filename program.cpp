...
{
	int cam_number, size, timecount, timeSeconds; //Initialize camera number, image size and time variables.
	int pixelcount1; //initialize variable where area value would be stored
	int pixelcount2; //initialize second variable where area value would be stored 
	int Sensitivity; //initialize sensitivity variable. It would hold the error value and make the program more or less sensitive to motion
	int height = 480, width = 640;
	char response;
	FILE *fp;
	char fileName[100];
	image backgroundcolor1, backgroundcolor2, backgroundgrey1, backgroundgrey2; //initialize couple images
	image color1, color2, grey1, grey2;
	image image_compare;

	ofstream outputFile; //this would allow the result of motion detection to be stored in an external file
	...
	while (1)
	{
		...
		acquire_image(backgroundcolor1, cam_number);    //Image acquisition and manipulation
		...
		{
			if (KEY('X')) exit(0);
			timecount += 4000; //time should increment by 4000 milli-seconds. Can be changed by user
			...
			if ((pixelcount1 != 0) && (pixelcount1 > Sensitivity)) //compare subtracted area with sensitivity value and outpu if there was motion or not
			{
				cout << "\a**** At time: " << timeSeconds << " seconds, there was motion around the 3D printer! ****\n\n" << endl;
				outputFile << "**** At time: " << timeSeconds << " seconds, there was motion around the 3D printer! ****\n\n" << endl;
				Sleep(4000); //update every 4000 milliseconds
			}
			else 
			{
				cout << "At time: " << timeSeconds << " seconds, there was no motion around the 3D printer!\n\n" << endl;
				outputFile << "At time: " << timeSeconds << " seconds, there was no motion around the 3D printer!\n\n" << endl;
				Sleep(4000);
			}

			copy(grey1, backgroundgrey2); //make the new image to become background, and the loop continues
		}		
	}
