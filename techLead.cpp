//The functions in this techlead.cpp and techlead.h were all created and written by the group members
...
double shape_detect(double ic, double jc, int label_number, image &label_image, image &rgb)
{
	...
	// Max radius angle from centroid
	...
	if ((r_max / min) > 1.414)printf("\n Shape: Features match a rectangle "); // Using trigonometry of the right triangle between the centroid, one edge and the middle of one side

	else
	{
		if ((max - min)<3)printf("\n Shape: Features match a circle "); //from testing. if the max and min radii are similar enough, we have a circle

		else printf("\n Shape: Features match a Square ");
	}
